function h = schemaball4(r, lbls, ccolor, ncolor)
...
%     ccolor=[zeros(1,N);sqrt(sin(linspace(1e-2,pi/2,N)));zeros(1,N)]';
%     ccolor=[flip(ccolor);ccolor];
end
% Ncolor
szN = size(ncolor);
...
ipos       = x > 0;
s.t        = zeros(sz(1),1);
s.t( ipos) = text(x( ipos)*1.1, y( ipos)*1.1, lbls( ipos),'Color',tcolor,'Fontsize',14);
