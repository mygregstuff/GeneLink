...
nor=round(length(names)/length(unique(names)));%number of replicates
od=zeros(size(data,2)/nor,nor,size(data,1));
for i=1:size(data,2)/nor
    for ii=1:nor
        od(i,ii,:)=data(:,i+ii-1);
...
if isnumeric(names)==1
    names=mat2cell(names,size(names,1),size(names,2)); %convert numerical names to cell
end
