...
[data,time,Data,names]=deal([]);
msgbox('Select the file with growth data')
...
for j=filename
    ...
    if files==1 && cell2mat(strfind(j,'xls')) 
        if data_select=='manual' 
        msgbox('sellect names')
        names = xlsread([filepath,j{:}],-1);
        msgbox('sellect time')
        [~, ~, time] = xlsread([filepath,j{:}],-1);
        msgbox('sellect blanks')
        [~, ~, blanks] = xlsread([filepath,j{:}],-1);
        msgbox('sellect raw growth data')
        [~, ~, od] = xlsread([filepath,j{:}],-1);
        else cell2mat(strfind(j,'xls')) ~=0 
...
msgbox({'Bootstrapping...' [num2str(nc) ' Samples ' num2str(bs) ' Times']})
...
for j=1:size(methods,2)
    ...
    for jj=1:size(metrics,2)
        ...
        [~,~,~,~,~,~,~,f,~,fsimilar,T]=dendqual5(t3,z,metrics{jj},methods{j},r,names,[filepath filename,'_cluster_vecs.csv']);
        r=1+r
